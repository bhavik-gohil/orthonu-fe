...
export default function CartItemRow({
    ...
    const nameSlug = item.productName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
    const productUrl = `/shop/product/${nameSlug}/${item.productSlug}`;
    const regularPrice = Number(item.regularPrice) ?? 0;
    const professionalPrice = item.professionalPrice != null ? Number(item.professionalPrice) : null;

    const isProfessional = userType === "professional";
    const price = isProfessional && professionalPrice != null ? professionalPrice : regularPrice;
    const showProfessionalPricing = isProfessional && professionalPrice != null;

    const lineTotal = price * item.quantity;

    return (
        <div className="flex flex-col md:flex-row gap-6 p-6 bg-white rounded-2xl border border-zinc-100 shadow-sm">
            {/* Product Image */}
                ...
                <div className="w-full md:w-32 aspect-square bg-zinc-50 rounded-lg overflow-hidden border border-zinc-100">
                    {item.productImage ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                            <ShoppingBag size={48} strokeWidth={1} className="text-zinc-200" />
                        </div>
                    )}
                </div>
            </Link>
            ...
            <div className="flex-1 flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                ...
                <div className="flex-1 space-y-2">
                    <Link href={productUrl} className="hover:text-brand-blue transition-colors">
                        <h3 className="text-lg font-black text-soft-dark tracking-tight leading-tight">
                            {item.productName}
                        </h3>
                    </Link>
                    <div className="flex items-center gap-2 flex-wrap">
                        {item.productCategory && (
                            <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-soft-dark/40">
                                {item.productCategory}
                            </p>
                        )}
                        {item.variantName && (
                            <span className="text-[9px] font-bold uppercase tracking-widest text-brand-blue bg-brand-blue/10 px-2 py-0.5 rounded-full">
                                {item.variantName}
                            </span>
                        )}
                        {item.color && (
                    ...
                    {showProfessionalPricing ? (
                        ...
                        <p className="text-sm font-medium text-soft-dark/60">
                            ${price.toFixed(2)} each
                        </p>
                    )}
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 bg-zinc-50 rounded-xl p-1 border border-zinc-200">
                        <button
                            ...
                            className="w-11 h-11 flex items-center justify-center rounded-lg hover:bg-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                            aria-label="Decrease quantity"
                        >
                            <Minus size={16} strokeWidth={2.5} />
                        </button>
                        <span className="w-12 text-center font-bold text-soft-dark">
                            {item.quantity}
                        </span>
                        <button
                            onClick={() => onUpdateQuantity(item.id, item.quantity + 1)}
